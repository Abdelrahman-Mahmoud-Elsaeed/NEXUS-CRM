import { MoreVertical, Pencil, Send, Eye, BarChart3, Plus, Trash2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
interface CompanyActionMenuProps {
  ...
  context: "list" | "grid";
  userRole: Role;
  onAction?: (actionType: string, companyId: string) => void;
}
...
export function CompanyActionMenu({
  ...
  const menuConfig = [
    ...
    },
    {
      id: "view_deals",
      label: "View Deals",
      icon: <BarChart3 size={18} className="text-on-surface-variant" />,
    ...
    },
    {
      id: "add_deal",
      label: "Add Deal",
      icon: <Plus size={18} className="text-on-surface-variant" />,
  ...
  return (
    <DropdownMenu  key={`${context}-${companyId}`}>
      ...
      <DropdownMenuContent
        align="end"
        sideOffset={4}
        className="bg-surface border border-outline-variant rounded-xl shadow-xl w-48 min-w-48 p-1 text-on-surface z-50"
      >
        {safeItems.map((item, index) => {
          ...
          return (
            <div key={item.id}>
              ...
              <DropdownMenuItem
                onClick={() => onAction?.(item.id, companyId)}
