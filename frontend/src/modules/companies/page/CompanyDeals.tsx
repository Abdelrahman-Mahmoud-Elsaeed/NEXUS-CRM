...
import { fetchDeals } from "@/modules/deals/store/deals.actions";
...
export function CompanyDeals() {
  ...
  const { items, isLoading } = useSelector((state: RootState) => state.deals);
  ...
  const companyDeals = items.filter((d: any) => d.company?.id === companyId);
  const activeDeals = companyDeals.filter((d: any) => d.status === "Open");
  const closedDeals = companyDeals.filter((d: any) => d.status !== "Open");
  const displayedDeals = activeTab === "active" ? activeDeals : closedDeals;
  ...
  const getStageColor = (stageName: string) => {
    const colors: Record<string, string> = {
      ...
      "Proposal Sent": "bg-secondary-container text-on-secondary-container",
  ...
  const totalPipelineValue = activeDeals.reduce((sum: number, d: any) => sum + (parseFloat(d.value || "0")), 0);

  return (
    <main className="ml-[260px] h-screen flex flex-col overflow-hidden relative">
      {/* Content Canvas */}
      <div className="flex-1 overflow-y-auto p-container-padding bg-surface-bright space-y-6">
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-12 md:col-span-4 glass-card p-6 rounded-xl">
            ...
            <div className="flex flex-col">
              <span className="font-h2 text-h2">{formatCurrency(totalPipelineValue.toString())}</span>
          ...
          <div className="px-6 py-4 border-b border-outline-variant bg-surface-container-low flex justify-between items-center">
            <div className="flex gap-4">
              <button
                ...
                className={`font-label-md text-label-md pb-4 ${
                  activeTab === "active"
                  ...
                  <tr key={deal.id} className="group hover:bg-surface-container-low/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span
                          ...
                          className="font-label-md text-label-md text-on-surface group-hover:text-primary transition-colors cursor-pointer"
                        >
                          {deal.name}
                        </span>
                    ...
                    <td className="px-6 py-4">
                      <div className="w-full bg-surface-container-highest rounded-full h-1.5 max-w-[100px]">
                        <div
                          className={`${getProbabilityColor(deal.stage?.name || "")} h-1.5 rounded-full`}
                          style={{ width: `${getProbability(deal.stage?.name || "")}%` }}
                        ></div>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end">
                        <div className="h-8 w-8 rounded-full overflow-hidden border-2 border-surface-container-lowest bg-primary-container flex items-center justify-center text-[10px] text-white font-bold">
                          {deal.assignee?.name?.split(" ").map((n: any) => n[0]).join("") || "?"}
                        </div>
                      </div>
                    </td>
        ...
        <div className="flex items-center justify-between text-on-surface-variant">
          <span className="font-body-sm text-body-sm">
            Showing {displayedDeals.length} {activeTab} deals
          </span>
          <div className="flex gap-2">
      ...
      <div className="absolute bottom-8 right-8 z-50">
        <button
          ...
          className="h-14 w-14 rounded-2xl bg-primary-container text-on-primary-container shadow-xl hover:shadow-2xl transition-all active:scale-95 flex items-center justify-center group"
        >
          <svg className="w-8 h-8 group-hover:rotate-90 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24">
