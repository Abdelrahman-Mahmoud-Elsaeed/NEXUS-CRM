...
interface MoveToDealModalProps {
  isOpen: boolean;
  onClose: () => void;
  contactName: string;
  companyName: string;
...
export function MoveToDealModal({ isOpen, onClose, contactName, companyName, onSubmit }: MoveToDealModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [apiError, setApiError] = useState<string | null>(null);
  const [dealName, setDealName] = useState(`${companyName || "Unknown"} - New Deal`);
      ...
      <section className="bg-white border border-outline-variant rounded-lg p-6 space-y-6">
        ...
        <div className="flex gap-4 p-4 bg-surface-container-low rounded-lg border border-outline-variant">
          <div className="flex-1">
            <Label className="font-label-sm text-on-surface-variant block mb-1">Contact</Label>
            <div className="flex items-center gap-2 font-body-base font-semibold text-primary">
