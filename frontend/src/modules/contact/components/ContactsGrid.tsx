...
export function ContactsGrid({ contacts, visibleLimit, totalRecords, onLoadMore }: ContactsGridProps) {
    ...
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
        {visibleContacts.map((contact) => {
          const currentStatus = contact.status?.toLowerCase() || "prospect";
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex flex-col items-end gap-4">
                  <span className={`px-2.5 py-1 text-[11px] font-bold rounded-full uppercase tracking-wider mb-2 ${getStatusClasses(contact.status)}`}>
              ...
              <div className="mb-4">
                ...
                <p className="text-body-sm text-outline">{contact.role || "Contact"}</p>
                <a className="text-label-sm text-primary font-bold hover:underline mt-1 inline-block" href="#">
                  {contact.company ? `${contact.company} Systems Inc.` : "Independent Vendor"}
                </a>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-outline-variant/30">
                <div className="flex flex-col">
                  ...
                  <span className="text-body-sm text-on-surface">{contact.lastActive || "Just now"}</span>
                </div>
